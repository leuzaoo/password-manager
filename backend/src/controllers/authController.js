...
export async function signup(req, res) {
  ...
  try {
    ...
    if (existingUser.rows.length > 0) {
      return res.status(400).json({message: "Email em uso. Use outro por favor."})
    }
    ...
    const user = await pool.query("INSERT INTO users (email, password) VALUES ($1, $2) RETURNING id, email", [email, hashedPassword]);

    const token = generateToken(user.rows[0].id, res);

    res.status(201).json({
      message: "Boas vindas ao aplicativo.", user: {...user.rows[0], password: ""}, token,
    });
  } catch (error) {
...
export async function login(req, res) {
  ...
  try {
    ...
    res.json({
      message: "Login realizado.", user: {...user.rows[0], password: ""}, token,
    });
  } catch (error) {
    res.status(500).json({error: error.message});
  }
